$('document').ready(function (event) {
    console.log("in update Faculty info");
    var str = {
        'id': +$('#id').val()
    };
    console.log(str);
    $.ajax({
        url: "http://localhost/SMS/api/faculty/faculty_by_id.php",
        type: "GET",
        datatype: 'JSON',
        data: str,
    ...
    $("#update_faculty").on("click",function(event){
        ...
        if(confirm("Are you sure, You want to update this details?"))
        {
            ...
            $.ajax({
                url: "http://localhost/SMS/api/faculty/updateFaculty.php",
                type: "POST",
                datatype: 'json',
                data: str,
                success: function (data) {
                    console.log(data);
                    if(data.status == 202){
                        location.replace("http://localhost/SMS/faculties.php");
    ...
    $("#cancel_faculty_update").on("click",function(){
